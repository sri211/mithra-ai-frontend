      ...
      <div style={{ maxWidth: "680px", margin: "0 auto", padding: "24px 16px", display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        <div style={{ textAlign: "center", padding: "32px 0 8px" }}>
          ...
          <p style={{ fontSize: "15px", color: "#64748b", lineHeight: 1.6, maxWidth: "400px", margin: "0 auto" }}>
            Refer 3 friends to Mithra AI and get <strong style={{ color: "#a78bfa" }}>1 month Pro free</strong> — worth ₹198.
          </p>
        </div>

        {/* Progress card */}
        <div style={{ background: "rgba(18,10,36,0.9)", border: `1px solid ${unlocked ? "rgba(16,185,129,0.3)" : "rgba(124,58,237,0.2)"}`, borderRadius: "20px", padding: "24px" }}>
          {isLoading ? (
            ...
            <>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
                <div>
                  <div style={{ fontSize: "32px", fontWeight: 900, color: unlocked ? "#10b981" : "#a78bfa" }}>{count} / {needed}</div>
              ...
              <div style={{ height: "8px", borderRadius: "4px", background: "rgba(255,255,255,0.06)", overflow: "hidden", marginBottom: "8px" }}>
                <motion.div
                  ...
                  style={{ height: "100%", borderRadius: "4px", background: unlocked ? "#10b981" : "linear-gradient(90deg,#7c3aed,#f59e0b)" }}
                />
              </div>
          ...
          {!!data?.referral_link && (
            <>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <div style={{ flex: 1, background: "rgba(15,8,30,0.8)", border: "1px solid rgba(124,58,237,0.2)", borderRadius: "10px", padding: "10px 14px", fontSize: "13px", color: "#a78bfa", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {String(data.referral_link)}
                </div>
                <button onClick={copyLink} style={{ display: "flex", alignItems: "center", gap: "5px", padding: "10px 16px", background: copied ? "rgba(16,185,129,0.15)" : "rgba(124,58,237,0.15)", border: `1px solid ${copied ? "rgba(16,185,129,0.3)" : "rgba(124,58,237,0.3)"}`, borderRadius: "10px", color: copied ? "#10b981" : "#a78bfa", fontSize: "13px", fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
                  {copied ? <><Check style={{ width: "14px", height: "14px" }} />Copied!</> : <><Copy style={{ width: "14px", height: "14px" }} />Copy</>}
                </button>
                <button onClick={shareLink} style={{ display: "flex", alignItems: "center", gap: "5px", padding: "10px 16px", background: "linear-gradient(135deg,#7c3aed,#6d28d9)", border: "none", borderRadius: "10px", color: "white", fontSize: "13px", fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
                  <Share2 style={{ width: "14px", height: "14px" }} />Share
                </button>
              </div>
              <div style={{ marginTop: "8px", fontSize: "11px", color: "#475569" }}>
                Code: <span style={{ fontFamily: "monospace", color: "#a78bfa", fontWeight: 700 }}>{String(data.referral_code)}</span>
              </div>
            </>
          )}
        </div>
        ...
        {/* How it works */}
        <div style={{ background: "rgba(18,10,36,0.9)", border: "1px solid rgba(124,58,237,0.15)", borderRadius: "20px", padding: "20px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {STEPS.map((s) => (
              <div key={s.n} style={{ display: "flex", gap: "14px", alignItems: "flex-start" }}>
                <div style={{ width: "36px", height: "36px", borderRadius: "12px", background: "linear-gradient(135deg,#7c3aed,#6d28d9)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontWeight: 900, fontSize: "14px", color: "white" }}>
                  {s.n}
                </div>
                <div>
